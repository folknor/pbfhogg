...
**Consumer-side `--snapshot` is incomplete** as of brokkr 2026-04-28: `sort`, `inspect`, `cat`, `add-locations-to-ways`, `getid`, `tags-filter` (non-OSC), `build-geocode-index`, `extract`, `multi-extract`, `check-refs` do not yet accept `--snapshot KEY`. To consume a snapshot from those, you have to point pbfhogg at the file directly.
