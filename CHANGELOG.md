...
- **apply-changes `--parallel-writer` backend (2026-04-21).** New writer backend: one coordinator thread + `POOL_SIZE=16` pwrite workers on a shared file descriptor. Writer pops pipeline items in global-seq order via the existing `ReorderBuffer`, computes each item's final byte offset, and round-robins `WriteOp` (Write { offset, bytes } | CopyRange { out_offset, in_fd, src_offset, len }) across per-worker bounded channels. Workers run `pwrite` / `copy_file_range(out_offset)` concurrently; cross-device copy_file_range (EXDEV) falls back to pread+pwrite with explicit offsets (parallel-safe). New CLI flag `pbfhogg apply-changes --parallel-writer` (mutually exclusive with `--direct-io` / `--io-uring`). Planet bench matrix at `--compression zstd:1` cross-disk: buffered 87.1 s → io_uring 82.8 s → **parallel-writer 80.9 s**. Pool-size sweep: 4 → 89.2 s, 8 → 83.4 s, 16 → 80.9 s, 32 → 82.2 s (NVMe queue saturated around 16). Writer-backend rule: io_uring wins same-disk (IOPS contention, queue-depth batching) and at `--compression none`; parallel-writer wins cross-disk + zstd:1 (bandwidth headroom, parallel pwrite saturates disk).
...
| apply-changes --locations-on-ways (daily diff, `--parallel-writer` + zstd:1, separate-NVMe scratch) | Planet 87 GB altw | 80.9 s | -89% (9.4×) vs 762 s in 0.2.0 |
