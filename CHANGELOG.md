...
- **`apply-changes --force` on a malformed non-indexed PBF could emit every remaining upsert of a kind as gap-creates before a single blob.** When the wire-level block-type classification (the first group's first tag byte) disagreed with the actual element kinds in the block, `streaming.rs::infer_kind_and_range` returned the reversed-range sentinel `(i64::MAX, i64::MIN)`. The drain then called `blob_osm_first_id` on that range, which returned `i64::MAX`, and `handle_gap_creates` emitted every remaining upsert of the block's claimed kind as a gap-create ahead of the blob's rewritten output. Drain-side `process_item` now detects `min_id > max_id` and skips the gap-creates call; trailing-creates picks up any legitimate upserts. Trigger is malformed wire input only; production writers don't produce it.
- **`apply-changes --force` non-indexed false-positive blobs credited `base_*` stats to the wrong kind and could flush upsert cursors prematurely.** On the `--force` path the scanner fills every `BlobDescriptor` with a placeholder `kind=Node` and `id_range=None` (no indexdata to consult). Workers that decompressed a blob, found it didn't overlap the diff (false-positive), and routed it straight through passthrough carried those placeholders into the drain. `stats.base_nodes` got credited with the element count of a Way/Relation blob, and when the next Rewritten item of the real kind arrived the drain ran a bogus `Node -> Way`/`Node -> Relation` type transition, triggering `flush_remaining_upserts` that drained upsert cursors before any real blob of that kind had been processed. Subsequent Rewritten blobs of that kind then found their inline upsert slice empty and the intended modifications were lost. The worker now walks the parsed block to recover the true kind + range before handing off to either the CopyRange false-positive path or the owned-passthrough path.
...
- **Parallel writer could silently truncate output on upstream framer panic.** Both the parallel-pwrite writer (`write/parallel_writer.rs::dispatch_loop`) and the io_uring writer (`write/uring_writer.rs::uring_main_loop`) broke out of the recv loop on channel close without checking that the `ReorderBuffer` was drained. If a rayon framing task panicked and dropped its sender without emitting an earlier seq, the gap chunks stayed pending, the writer returned `Ok(())`, and the output file closed short with no diagnostic. Both loops now surface the gap as an error ("channel closed with N item(s) still in reorder buffer").
- **`add-locations-to-ways --index-type external --force` on a non-indexed PBF failed with a confusing "OsmData blob missing indexdata" error** deep inside the metadata scan. External-join's rank-based bucket ranges depend on per-blob indexdata, so `--force` cannot meaningfully apply to this path. The combination is now rejected up front with a clear migration hint: use `pbfhogg cat` to produce an indexed PBF first, or switch to `--index-type dense`/`sparse` which tolerate `--force`.
- **Parallel `diff` / `derive-changes` leaked per-shard scratch temp files on worker error or panic.** If a sharded `diff -j N` or `derive-changes -j N` run hit an error or panic mid-phase, the main-thread concatenate loop's `?` propagated the first failure and abandoned every already-successful shard's `.txt.tmp` / `.xml.tmp` file on disk. On repeated failures, `scratch_dir` accumulated `diff-par-*-{pid}-*` / `derive-par-*-{pid}-*` files. Both drivers now sweep successful and panic-induced temp files before returning the error.
...
- **`add-locations-to-ways --index-type external` hit `EMFILE` on a default-ulimit shell.** Stage 1 pass B opened `num_workers * NUM_BUCKETS` (~4352 fds at 17 workers) rank-shard files concurrently; Linux default soft `RLIMIT_NOFILE` of 1024 meant the command failed immediately with `create rank shard ...: Too many open files (os error 24)` on any fresh terminal. The command now self-raises `RLIMIT_NOFILE` soft to hard cap at startup (unprivileged, free) and caps `num_workers` so the shard fleet always fits under whatever fd budget results. If even a single worker's 256-shard budget cannot fit, the command fails clean with a `ulimit -n N` hint rather than crashing mid-run. Adds three brokkr counters (`extjoin_nofile_soft_cap`, `extjoin_cpu_cap_workers`, `extjoin_fd_cap_workers`) so the effective cap is visible in benchmark output.
