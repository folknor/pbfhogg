...
- **diff**: shard-based parallel block-pair merge, opt-in via `PBFHOGG_DIFF_SHARDS=N` on `pbfhogg diff`. Planet two-snapshot diff 2134s (35m34s) → 235s (3m54s) at 16 shards (**9.1×**); Germany 103s → 17s at 8 shards (6.1×). Pre-pass walks both files' indexdata to partition the ID space into N independent shards, then `std::thread::scope` workers pread/decode/merge each shard and buffer formatted output; main thread concatenates shard outputs in order. Peak RSS 2.4 GB at planet. Only the `diff` (human-readable) path is parallelised; `diff --format osc` still uses the sequential `derive_changes`.
