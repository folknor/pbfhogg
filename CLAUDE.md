...
- `scripts/bench-commands.sh <cmd> [pbf] [runs]` — CLI command benchmark vs osmium (logs to benchmarks/benchmarks-commands.tsv). Commands: cat-way, cat-relation, tags-count, tags-count-way, tags-filter-way, tags-filter-amenity, tags-filter-twopass, getid, removeid, add-locations-to-ways, node-stats, all
