...
- `brokkr results [--commit X] [--compare A B] [--compare-last] [--command CMD] [--mode M] [--grep STR] [--dataset D] [--meta K=V] [-n N] [--top N]` — query/compare from SQLite. `--command` exact-matches the bare subcommand id (no `bench `/`hotpath ` prefix); `--mode` filters by measurement mode (`bench`/`hotpath`/`alloc`); `--grep STR` substring-matches against both `cli_args` and `brokkr_args` (use it to find runs by flag/axis, e.g. `--grep zstd:1` or `--grep snap-20260411`); `--dataset` substring match on input file; `--meta` exact match, composable with AND.
