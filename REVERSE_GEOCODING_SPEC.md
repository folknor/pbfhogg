...
| 16 | `f32` | Search radius in meters (default 75.0) |
| 20 | `u32` | Replication sequence number (0 if unknown) |
| 24 | `u64` | Replication timestamp (unix seconds, 0 if unknown) |
| 32 | `u32` | Address point count |
| 36 | `u32` | Street way count |
| 40 | `u32` | Interpolation way count |
| 44 | `u32` | Admin polygon count |
| 48 | `u32` | Geo cell count (street-level) |
| 52 | `u32` | Coarse cell count |
| 56 | `u32` | Admin cell count |
| 60 | `[u8; 4]` | Reserved (zero) |
...
For interpolation, the reader needs way-level context (total length,
start/end house numbers) to compute the interpolated address. The query path:
1. Read 2 nodes for the matched segment (distance check).
2. If this is the nearest interpolation hit, read the way header from
   `interp_ways.bin` for start/end numbers and total node count.
3. Compute accumulated distance from way start to the snap point by reading
   nodes from the way start to the matched segment. This is O(segment_index)
   node reads, but only happens for the single nearest interpolation match.
...
geometry is assigned cells at both levels. The coarse index has fewer cells
(level 14 has ~8x fewer cells than level 17 per unit area) but more entries
per cell, so the total size is modest: roughly 15-20% of the fine index.
...
| coarse_*_entries.bin | ~5 MB | ~1 GB |
...
These are rough estimates. The planet estimate is ~3.3x larger than
traccar-geocoder's 18 GB. The main cost drivers vs traccar-geocoder:
segment-level street entries (6 bytes each, ~1.5B entries = ~9 GB vs their ~3 GB
for whole-way entries), postcode per address point (20 vs 16 bytes), wider
record formats (u16 node counts, u64 node offsets), and the coarse fallback
index. The tradeoff is accepted: segment indexing eliminates per-way polyline
iteration on the query path, and the coarse index covers rural areas. Actual
sizes will be validated on Denmark during implementation.
...
impl Reader {
    ...
    /// Equivalent to `self.candidates(lat, lon).into_result()`.
...
/// Raw candidates from a spatial lookup. Unranked.
pub struct Candidates<'a> {
    /// All address points within search radius, with distances.
    pub addresses: ArrayVec<AddressMatch<'a>, 32>,
    /// All street segments within search radius, with distances.
    pub streets: ArrayVec<StreetMatch<'a>, 32>,
    /// All interpolation segments within search radius, with distances.
    pub interpolations: ArrayVec<InterpolationMatch<'a>, 16>,
    /// All admin polygons containing the query point.
    pub admin: ArrayVec<AdminMatch<'a>, 10>,
}
...
impl<'a> Candidates<'a> {
    ...
    pub fn into_result(self) -> ReverseResult<'a>;
}
```

The `Candidates` ArrayVec capacities (32, 32, 16) are generous upper bounds
for a 9-cell neighborhood. If a query returns more candidates than the capacity,
only the nearest N are kept (the candidate collection loop tracks a distance
threshold and evicts the farthest entry when full).

`query()` is a convenience wrapper: `self.candidates(lat, lon).into_result()`.
...
`coarse_level` (default 14). The 9-cell neighborhood at level 14 covers ~1.8
km, reaching streets in rural areas. The coarse search uses the same scoring
logic and references the same underlying data files.

This two-level approach keeps the common case fast (urban queries hit level 17
only) while covering rural areas without enumerating exponentially more cells.
The search radius (`max_distance_sq`) is not changed — the coarse level simply
provides more candidates to evaluate, and the distance filter still applies.
