...
- [ ] **[notes/diff-snapshots-opportunities.md](notes/diff-snapshots-opportunities.md)** - `diff-snapshots` (both human-readable and `--format osc`). Planet baseline **2134 s / 35m34s** (UUID `a9d430f2`, commit `052da8b`, 2026-04-19). **Shard-based parallel block-pair merge landed 2026-04-20**: planet **234.7 s / 3m54s at 16 shards (9.1× speedup)**, germany **17.0 s at 8 shards (6.1×)**. Opt-in via `PBFHOGG_DIFF_SHARDS=N` env var on `pbfhogg diff`. Utilization NODE 92 %, WAY 81 %, peak RSS 2.4 GB at planet. Beats the 8-min aspirational target. Open: (1) promote to a CLI flag, (2) pread-only walker (cold walker is 14 % of planet wall), (3) generalise to `derive_changes` (`--format osc`), (4) decide whether to auto-enable parallel by default. Items 3 (per-element cost reductions) and 4-7 (v3/byte-equal/alloc/io) in the plan remain closed/deferred - parallel blob-pair merge was the whole win.
