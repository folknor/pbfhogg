...
- [x] ~~**[notes/repack.md](notes/repack.md)** - new command: re-encode a PBF with a configurable `--elements-per-blob N` cap.~~ **v1 LANDED.** Per-kind parallel scan mirrors `cat --clean`; `BlockBuilder::with_element_cap(n)` added as the cap-plumbing primitive; CLI flags `--elements-per-blob`, `--compression`, `--direct-io`, `--io-uring`, `--force`. v1 fires the cap per worker invocation, so growing beyond the input blob size (e.g. 8 k -> 64 k) emits at-most-input-sized output blobs; cross-input-blob coalescing is deferred to v2. Planet 8 k repack still needs to run (long, single-shot) to unblock the blob-density measurement matrix and the `getparents` decision below.
  ...
  **Release / measurement follow-ups:**
  - [ ] **Drop the "Unreleased" callout from `docs/cli/commands.md` before 0.4.0 ships.** The `> **Unreleased** - lands in the next pbfhogg release after 0.3.0.` line in the repack section was added so the docs site distinguishes shipped from in-tree behavior. Remove it as part of the 0.4.0 release checklist (alongside the `Version 0.2.0` header re-stamp on both `docs/cli/commands.md` and `reference/cli-reference.md`).
  - [ ] **Plant the planet 8 k repack baseline.** Run `brokkr repack --dataset planet --elements-per-blob 8000 --bench 1` (long, single-shot) to produce the 8 k-packed planet PBF that unblocks the blob-density measurement matrix in `reference/blob-density.md` and the deferred `getparents` HeaderWalker dispatch decision (europe-regressing / planet-winning along the same blob-count axis). Capture wall, peak anon RSS, and output blob count; the resulting PBF is also the input for every same-corpus-different-encoding pair the matrix needs.
...
- [ ] **[notes/merge-changes.md](notes/merge-changes.md)** - `merge-changes` (squash N OSCs → 1). The serial-across-inputs CLI shape `merge_changes::write_streaming` parallelizes cleanly via `IdSet::set_atomic_if_new` newer-wins dedupe. **Baseline landed 2026-04-26** (commit `16e3694`): planet 7-OSC = 267.2 s (UUID `bef0f1fa`), planet 1-OSC = 43.1 s (UUID `76f78e8b`); per-OSC scaling is essentially linear (7-OSC = 6.2× 1-OSC) so the parse phase dominates the wall. **Hotpath + alloc captured 2026-04-27 overnight at `4fc8e35`** (UUIDs `ee108ec9` hotpath / `13615a4a` alloc): 100 % of wall in 7 calls to `parse_osc_streaming`, per-OSC avg 37.8 s / **P95 50.9 s**, alloc 62.4 GB cumulative (entirely in `parse_osc_streaming`). The P95 tightens the parallel-parse ceiling from "~38-50 s" to ~51 s on this 4914..4920 range; estimated win **~210-215 s at planet 7-OSC scale**. No win at 1-OSC scale. Sizing detail in `notes/merge-changes.md` and the `reference/performance.md` Merge-changes section. Per-input `MERGECHANGES_PARSE_{START,END}` markers and `merge_changes_input_bytes` counter shipped 2026-04-22 (commit `4e3c7ea`). Library-level `osc::load_all_diffs` is **unused in production** (only `#[cfg(test)]` call sites); apply-changes takes a single OSC via `parse_osc_file` so the "per-input" axis doesn't apply there. Next: implement the rayon-scoped parse-fan-out. Content factored out of `apply-changes-opportunities.md` 2026-04-21 - these items were filed under "weekly apply-changes" but apply scale-independently to any consumer that squashes N > 1 OSCs.
...
- [ ] **[notes/getparents.md](notes/getparents.md)** - `getparents` (whole-file scan listing ways / relations referencing a given ID set). Drafted 2026-04-23. **Already uses modern primitives** (pipelined decode, `BlobFilter` node-only-blob skip per CHANGELOG's ~85 % claim, `IdSet` chunked sparse bitset); not "never optimized". Headline opportunity is a **`HeaderWalker` + `IdSet::any_in_range()` blob-level fast path** mirroring `getid`'s include mode (planet 44 s → 7 s, 6.2x); estimated 4-8x at planet, 1-2 days scope, requires indexdata with a fallback. `parallel_classify_phase` substitution is bench-gated (10-20 % or neutral; keeps the c912e4d Denmark 4.7× sequential-decode regression explicitly off the table - that rule targets sequential-decode conversions, not pread-worker substitutions). Planet hotpath + alloc captured 2026-04-27 overnight at `4fc8e35` (UUIDs `00253c7d` hotpath / `5bec1e28` alloc): **23.0 s wall, 78 % in `parallel_classify_phase`** (17.97 s of 22.97 s), 169 s cumulative across decompress workers; alloc dominated by `decompress_blob_raw` (60 %) + `parse_and_inline_with_scratch` (26 %). Confirms read-path-bound shape and that the HeaderWalker fast-path would skip the 17,981 decompress calls almost entirely on indexed input - sizing the headline opportunity at the predicted 4-8× directly.
