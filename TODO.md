...
- [ ] **`diff --format osc` metadata fidelity - joint look with brokkr dev** (2026-07-10): the verify roundtrip check finds a stable set of nodes at `v0` after applying pbfhogg's derived OSC, on every encoding including primary. pbfhogg-diff-based comparison legs pass (metadata-blind by design, DEVIATIONS.md "diff: content equality"), so the version loss is invisible to our own tooling. DEVIATIONS.md "derive-changes: lossless delete roundtrip" claims a perfect roundtrip - that claim was presumably verified with pbfhogg's own metadata-blind diff and needs re-verification under metadata-strict comparison. Question: does OSC emission (or apply) drop version attributes for some elements? If expected, brokkr's check gains a version-only carve-out (their classify_diff precedent); if not, it's a pbfhogg bug.
