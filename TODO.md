...
- [ ] **[notes/merge-changes.md](notes/merge-changes.md)** - `merge-changes` (squash N OSCs → 1). The serial-across-inputs CLI shape `merge_changes::write_streaming` parallelizes cleanly via `IdSet::set_atomic_if_new` newer-wins dedupe. **Baseline landed 2026-04-26** (commit `16e3694`): planet 7-OSC = 267.2 s (UUID `bef0f1fa`), planet 1-OSC = 43.1 s (UUID `76f78e8b`); per-OSC scaling is essentially linear (7-OSC = 6.2× 1-OSC) so the parse phase dominates the wall. **Hotpath + alloc captured 2026-04-27 overnight at `4fc8e35`** (UUIDs `ee108ec9` hotpath / `13615a4a` alloc): 100 % of wall in 7 calls to `parse_osc_streaming`, per-OSC avg 37.8 s / **P95 50.9 s**, alloc 62.4 GB cumulative (entirely in `parse_osc_streaming`). The P95 tightens the parallel-parse ceiling from "~38-50 s" to ~51 s on this 4914..4920 range. **Pre-flight gzip-vs-XML and per-OSC change-count instrumentation landed 2026-04-28** (commit `fb1719c`); re-bench UUID `c612c5e6` proved **gzip = 1.5 % of wall** (3.98 s of 272.6 s), killing the parallel-decompress alternative and locking in the buffer-and-drain shape. Revised estimate: **~80-115 s ceiling, ~160-190 s win, ~2.4-3.4x speedup** (the prior 210-215 s assumed the drain pass was free, but decoupling parse from output introduces a new serial cost: re-walking 26.26 M changes to emit XML and gzip-compress 968 MB output, ~30-60 s estimated). If drain dominates, swap output through `ParallelGzipWriter`. No win at 1-OSC scale. Sizing detail in `notes/merge-changes.md` and the `reference/performance.md` Merge-changes section. Per-input `MERGECHANGES_PARSE_{START,END}` markers and `merge_changes_input_bytes` counter shipped 2026-04-22 (commit `4e3c7ea`); `merge_changes_decompress_ns` and `merge_changes_changes_per_osc` shipped 2026-04-28 (`fb1719c`). Library-level `osc::load_all_diffs` is **unused in production** (only `#[cfg(test)]` call sites); apply-changes takes a single OSC via `parse_osc_file` so the "per-input" axis doesn't apply there. Next: implement the buffer-and-drain parallel parse-fan-out. Content factored out of `apply-changes-opportunities.md` 2026-04-21 - these items were filed under "weekly apply-changes" but apply scale-independently to any consumer that squashes N > 1 OSCs.
