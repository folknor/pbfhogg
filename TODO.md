...
- [ ] **apply-changes drops OSC element metadata** (found 2026-07-10 while resolving the item above): `CompactDiffOverlay` never captures `version`/`timestamp`/`changeset`/`uid`/`user` from OSC XML (the parser doesn't read those attributes at all), so every OSC-sourced created/modified element in apply-changes output carries `v0` and no timestamp - byte-proven: the real seq-4705 OSC carries `version="2" timestamp="2026-02-20T21:39:49Z"` for n5900269010, the merged output has `v0 c0 t i0 u`. Invisible to our own tooling because pbfhogg diff is metadata-blind by design (DEVIATIONS.md). **Production-relevant**: the steady-state daily merge accumulates metadata-less elements over time (osmium applychanges preserves them). Fix is a real feature: plumb metadata through the overlay arenas (`osc/parse.rs`, `osc/compact.rs`) into `rewrite_block`/`element_writes` and the BlockBuilder add calls. Also re-verify DEVIATIONS.md's "derive-changes: lossless delete roundtrip" claim once fixed - "perfect roundtrip" is only true under metadata-blind comparison today.
