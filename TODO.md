...
- [x] ~~**[notes/repack.md](notes/repack.md)** - new command: re-encode a PBF with a configurable `--elements-per-blob N` cap.~~ **v1 LANDED.** Per-kind parallel scan mirrors `cat --clean`; `BlockBuilder::with_element_cap(n)` added as the cap-plumbing primitive; CLI flags `--elements-per-blob`, `--compression`, `--direct-io`, `--io-uring`, `--force`. v1 fires the cap per worker invocation, so growing beyond the input blob size (e.g. 8 k -> 64 k) emits at-most-input-sized output blobs; cross-input-blob coalescing is deferred to v2. Planet 8 k repack still needs to run (long, single-shot) to unblock the blob-density measurement matrix and the `getparents` decision below.
  ...
  **v1.1 follow-ups (small, no benchmarking):**
  - [ ] **Cap-message parity for `BlockBuilder::with_element_cap`.** The CLI rejects `--elements-per-blob 0` with `Err("--elements-per-blob must be > 0")`, but `BlockBuilder::with_element_cap(0)` panics with `"BlockBuilder element cap must be > 0"`. Same invariant, two strings. Align the assertion text so a grep across both surfaces collapses to one match. Pure text edit, no semantic change (panic stays panic).
  - [ ] **Warn on no-op grow cap.** When `--elements-per-blob > max(observed input blob size)`, v1 emits at-most-input-sized output blobs - the user asked for denser packing and got the same layout back. Run a single-pass tally during the per-kind scan and emit a stderr warning at end-of-run if the cap never fired on any worker (`"--elements-per-blob N exceeds the largest input blob (M elements); output blob layout unchanged. v1 cannot grow blobs across input boundaries."`). Keeps the silent-identity surprise out of the planet-density matrix workflow.
