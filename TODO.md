...
- [x] ~~**getid include mode**~~ - landed 2026-04-20 via a shared `pread`-only `HeaderWalker` primitive (`src/read/header_walker.rs`). Planet **43.7 s → 6.1 s (7.2×, UUID `24362e36`)**, germany 200 ms, disk read 88 GB → 601 MB. Initial HeaderWalker landing hit 7.0 s with two preads per blob; the follow-up 1-pread probe walker (commit `d263d76`) trimmed a further 0.9 s (-13 %). Walker is syscall-bound; going lower would need io_uring batching - not pursued. Plan doc retired. **2026-07-10 blob-density follow-up**: full three-cell matrix measured - europe HW 40.2 s (`57ffbf49`) vs scan 17.9 s (`bc96d15d`), planet-8k HW 102.6 s (`aa5bc158`) vs scan 33.2 s (`c0d89d8f`); HW regresses +125 % / +209 % on high-blob-count encodings while the scan arm actually IMPROVES with density (indexdata prescreen selectivity). getid joins getparents and sort pass 1 as the third consumer of the pending blob-count threshold-dispatch, and its steep divergence argues for a ~150 k-blob constant (see `reference/blob-density.md` "getid include mode confirms, steeper").
