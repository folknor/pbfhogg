...
| `diff` | `diff` | Content equality instead of version ordering (see [DEVIATIONS](/cli/deviations#diff-content-equality-vs-version-ordering)) |
...
from both input files (see [DEVIATIONS](/cli/deviations#derive-changes-lossless-delete-roundtrip)).
...
| `--index-type` | `sparse` (default), `external` (bounded memory, sequential I/O, only mode that survives at planet on memory-constrained hosts), `auto` (scale-aware: sparse unless the input is sorted+indexed and the estimated node store exceeds ~80 % of available RAM). Different valid values from osmium's `-i, --index-type`. |
