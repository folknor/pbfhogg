...
### #9 - Pull relation-member collection forward into stage 1 - **Layer 1 LANDED 2026-04-17 (commit `6d71053`); Layer 2 LANDED 2026-04-21 (commit `3b6b409`)**

**Layer 2 landed-result.** Europe `--bench 1` 279.6 s vs Commit B-fix baseline 292.2 s = -12.6 s / -4.3% wall. Planet `--bench 1` 614.2 s vs 652.4 s = -38.2 s / -5.9% (structural saving ~6.5 s from serialized relation scan elimination; rest is bench-1 noise across stages 1/2/streaming). Relation scan runs as a separate `scope.spawn` alongside `stage1_way_pass` inside a new `thread::scope` in `external_join`; both read the input PBF via pread (`File: Sync` on Unix). At planet the relation scan takes 16.5 s of wall and fits entirely inside stage 1's 114 s window. Peak anon RSS regressed +2.4 GB at planet (15.66 -> 18.1 GB) because `relation_member_node_ids` now lives from stage 1 onwards instead of only through stage 4; still has headroom on the 25 GB-available-RAM host. Closure return type is `Result<_, String>` because `BoxResult`'s error type is not `Send` (same pattern the #2 streaming scope uses). Denmark external byte-for-byte identical (52489653 nodes read, 3513255 written, 0 missing locations).
