...
Status: spec, 2026-07-11, amended same day after the cross-repo verdicts.
Landing 1 LANDED (commit `58743ba`: the format/reader layer, plus the D5
cap - later corrected to the 32 MiB reference-reader bound in `2387bbf` -
and the D7 OwnedBlock struct conversion). BOTH cross-repo gates CLOSED
2026-07-11 (see the amended "Gate" section): Brick 1 superset screen
PASSED and D9 RATIFIED as specified, so landings 2-4 are unblocked and
executable. Landing 5's flag-ON bench row keeps its two prerequisites
that were never the elivagar gate: the brokkr `--inject-prepass`
passthrough brick (Out of scope, named) must land first, and the planet
flag-on run needs an explicit user green-light. The flag-OFF neutrality
and docs/ADR parts of landing 5 have no such block.
...
**BOTH cross-repo gates CLOSED 2026-07-11.** elivagar's verdicts are
recorded normatively in their `notes/injected-prepass-spec.md`
"Cross-repo ratifications (2026-07-11)" section (their commit `0f67f51`;
instrumentation `4ceacd1`). Landings 2-4 are unblocked. The paragraphs
below record what the gates were and how they resolved; the extension
points they named are permanently closed, not deferred.

**Gate 1, Brick 1 superset screen: PASSED.** Measured on plantasjen at
elivagar `4ceacd1`: germany locations superset 1,092,149 vs needed
886,109 = **1.23x** (threshold 1.5x; their bench `f2b93719`); denmark
1.16x (`fc55fca5`). The zero-coupling superset semantics stand as
specified; the relation `tag_expr` filter is NEVER built, and the
"Membership scan" section stands unamended with its hardcoded
`type=multipolygon|boundary` check. elivagar keeps one caveat on their
own books: the count ratio is the early screen; their Brick 4 still
gates the real cost in bytes (exact-plan baseline at that run: 112.3 MB
way_index data + 14.1 MB index on germany locations).

**Gate 2, D9 resolved-refs refinement: RATIFIED as specified** (pin =
shared AND resolved). Their grounds: (1) both of their release-checked
length validations depend only on lengths, which D9 never changes -
bit values only, never bitmap length, presence rules, or coord/ref
alignment; (2) the unrefined semantics were actively wrong for them -
their enriched path maps bit i positionally onto `Way::node_locations()`
output, so a shared-but-absent node is a (0,0) sentinel vertex, and
pinning it would force DP simplification to RETAIN garbage; (3) it
matches their node-store path, where pins never land on unresolved refs
today. Consequence: landings 2-5 encode the narrowed semantics, the
landing-4 oracle is written to the ratified definition, and the
survey-faithful fallback named in D9 (zero-coord `ResolvedEntry`
emission on shared orphan runs) is DECLINED and closed - nothing on
their side ever wants a pinned position without a real coordinate.

**Also ratified: `Blob::way_member_count()` accepted** (the D8
count-validation fold). elivagar's Brick 4 validation grows a third
release-checked hard error comparing the encoded `way_count` against
the blob's actual decoded Way element count. Obligation on landing 4:
the oracle fixture set MUST include a within-byte count difference
(encoded vs actual way counts that round to the same bitmap byte
length) so that compare is exercised end-to-end.

**Landing-1 ordering vs the survey (historical).** Landing 1 landed
2026-07-11 (`58743ba`) as the documented additive/default-off exception
to the survey's original ordering; with both gates now closed the
exception is moot.
...
**Precedence conflict: RESOLVED 2026-07-11 - elivagar RATIFIED option (a)**
(see the Gate section for their full grounds and the record pointer). The
contract definition is now pin = shared AND resolved; the survey's
shared-ness text reads as narrowed to resolved positions, and the closure
trailing duplicate still mirrors bit 0 by construction (it resolves iff
position 0 does). Option (b), the zero-coordinate `ResolvedEntry` emission
for shared orphan runs, is DECLINED and closed - do not implement it. The
landing-4 oracle is written to the ratified definition, and per the D8
acceptance it must also include a within-byte encoded-vs-actual way-count
fixture (see Gate).
...
1. Oracle roundtrip test (Brick 2's contract gate) in
   ...
   in-test implementation of the ratified contract semantics (pin = shared
   AND resolved, per the 2026-07-11 D9 ratification), compare bitmaps
   bit-for-bit. The topology MUST include a way
