...
Run the seven steps in order (step 3 closes with a spec-commit, 3b). Never skip
a step, never reorder, never merge two steps into one launch. Use the prompts below verbatim - substitute X and Y,
...
> Read X, reference/technical-implementation-spec.md, and reference/testing.md
> and write a new implementation spec document. Do not commit. Do not launch
> any sub-agents.
...
Launch one Agent(sonnet), background:
...
> point durable references at the history ledger instead. Report each
...
1. `brokkr fmt`
2. `brokkr check`
3. `git rm` the spec document (tracked since step 3b; the orchestrator manages
   spec-doc lifecycle; durable findings settle into the history ledger per repo
   convention). The removal lands inside the same commit as the code.
4. Commit (the code plus the spec removal, one commit)
