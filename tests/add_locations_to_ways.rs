...
fn basic_locations_added_to_ways() {
    ...
    let stats = add_locations_to_ways(&input, &output, true, Compression::default(), false, true, &pbfhogg::HeaderOverrides::default(), pbfhogg::add_locations_to_ways::IndexType::default(), false, None).expect("add locations");
...
fn header_has_locations_on_ways_feature() {
    ...
    write_test_pbf(&input, &test_nodes(), &test_ways(), &[]);
    add_locations_to_ways(&input, &output, true, Compression::default(), false, true, &pbfhogg::HeaderOverrides::default(), pbfhogg::add_locations_to_ways::IndexType::default(), false, None).expect("add locations");
...
fn drop_untagged_nodes() {
    ...
    let stats = add_locations_to_ways(&input, &output, false, Compression::default(), false, true, &pbfhogg::HeaderOverrides::default(), pbfhogg::add_locations_to_ways::IndexType::default(), false, None).expect("add locations");
...
fn basic_locations_added_sparse() {
    ...
    let stats = add_locations_to_ways(&input, &output, true, Compression::default(), false, true, &pbfhogg::HeaderOverrides::default(), pbfhogg::add_locations_to_ways::IndexType::Sparse, false, None).expect("add locations");
    assert_eq!(stats.ways_written, 1);
    assert_eq!(stats.missing_locations, 0);
