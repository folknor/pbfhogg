...
fn basic_locations_added_to_ways() {
    ...
    let stats = add_locations_to_ways(&input, &output, true, Compression::default(), false, IndexType::Hash, true).expect("add locations");
...
fn header_has_locations_on_ways_feature() {
    ...
    write_test_pbf(&input, &test_nodes(), &test_ways(), &[]);
    add_locations_to_ways(&input, &output, true, Compression::default(), false, IndexType::Hash, true).expect("add locations");
...
fn drop_untagged_nodes() {
    ...
    let stats = add_locations_to_ways(&input, &output, false, Compression::default(), false, IndexType::Hash, true).expect("add locations");
...
fn dense_basic_locations_added_to_ways() {
    ...
    let stats = add_locations_to_ways(&input, &output, true, Compression::default(), false, IndexType::Dense { capacity: 1000 }, true).expect("add locations");
    assert_eq!(stats.ways_written, 1);
    assert_eq!(stats.missing_locations, 0);
...
fn dense_drop_untagged_nodes() {
    ...
    let stats = add_locations_to_ways(&input, &output, false, Compression::default(), false, IndexType::Dense { capacity: 1000 }, true).expect("add locations");
