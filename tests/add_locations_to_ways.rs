...
fn basic_locations_added_to_ways() {
    ...
    let stats = add_locations_to_ways(&input, &output, true, Compression::default(), false, true).expect("add locations");
...
fn header_has_locations_on_ways_feature() {
    ...
    write_test_pbf(&input, &test_nodes(), &test_ways(), &[]);
    add_locations_to_ways(&input, &output, true, Compression::default(), false, true).expect("add locations");
