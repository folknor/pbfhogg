...
/// Pool-worker panic inside `write::parallel_writer` (apply-changes'
/// default output backend) must surface as a hard failure, not a
/// silent short file. Uses the feature-gated static atomics in
/// `parallel_writer::test_hooks` to arm a panic on a specific
/// dispatched `WriteOp`.
///
/// **`#[ignore]`** because the hook is a process-global static and
/// races with any sibling test that also uses `parallel_writer`
/// (every apply-changes test does). Run via
/// `brokkr test fault_injection_parallel_writer_pool_panic_surfaces_error`,
/// which forces `--test-threads=1`. The hook shape was chosen over
/// a per-instance `MergeOptions` field because pool workers are
/// spawned deep inside `parallel_writer_thread_inner` - threading a
/// config through every entry point would be invasive. The
/// serialization cost is paid here, at the test site, rather than
/// in production code.
///
/// Note: `parallel_writer` assigns absolute `pwrite` offsets to every
/// op up-front. If a pool worker panics holding queued ops in its
/// channel, those ops' offsets go unwritten - the kernel leaves
/// zero-filled holes at those byte ranges. The command must still
/// return an error and must not claim success; that's the invariant
/// this test locks. Whether the resulting file is truncated or
/// merely malformed-and-holey is a secondary concern covered by
/// CHANGELOG notes, not by the absence-or-truncation assertion
/// (which is too strong for this backend).
#[cfg(feature = "test-hooks")]
#[test]
#[ignore]
fn fault_injection_parallel_writer_pool_panic_surfaces_error() {
    ...
    // Static hooks are process-global; reset before and after to
    // avoid leaking state to sibling tests. `--test-threads=1`
