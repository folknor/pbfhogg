...
/// 60 nodes / cap 10 = 6 node blobs (input 20/blob is an exact 2x of
/// cap, so per-input-blob fragmentation is zero). 12 ways / cap 10 = 2
/// blobs (1 full + 1 partial of 2). 3 relations / cap 10 = 1 partial.
#[test]
...
fn repack_large_cap_preserves_input_blob_layout() {
    ...
    // Fixture has 3 node blobs + 1 way blob + 1 relation blob = 5 input
    // OsmData blobs; output mirrors that count.
    let per_blob = elements_per_blob(&output);
    let node_blobs = per_blob.iter().filter(|(n, _, _)| *n > 0).count();
    let way_blobs = per_blob.iter().filter(|(_, w, _)| *w > 0).count();
    let rel_blobs = per_blob.iter().filter(|(_, _, r)| *r > 0).count();
