...
<osmChange version="0.6">
  ...
  <delete>
    ...
    let stats = merge(&base, &osc, &output, Compression::default(), false, false, false, true).expect("merge");
...
<osmChange version="0.6">
  <create>
    ...
    merge(&base, &osc, &output, Compression::default(), false, false, false, true).expect("merge");
    let c = read_all_elements(&output);
...
fn merge_cross_validate_osmium() {
    ...
    merge(&base_pbf, &osc, &pbfhogg_out, Compression::default(), false, false, false, true).expect("pbfhogg merge");
